...
describe CoralBackup do
  ...
  describe CoralBackup::FileSelector do
    ...
    let(:files) { file_selector.instance_variable_get(:@files) }
    ...
    describe "#add_file" do
      it "should add file" do
        message = "#{path1}\n"
        expect{ file_selector.send(:add_file, path1) }.to output(message).to_stderr
        expect(files).to contain_exactly(path1)
      end

      it "should add directory" do
        Dir.mktmpdir do |dir|
          message = dir + "\n"
          expect{ file_selector.send(:add_file, dir) }.to output(message).to_stderr
          expect(files).to contain_exactly(dir)
        end
      end

      it "should reject nonexistent files" do
        expect { file_selector.send(:add_file, nonexistent_path) }.to raise_error Errno::ENOENT
      end

      it "should allow to add multiple files" do
        message = path1 + "\n" + path2 + "\n"
        expect {
          file_selector.send(:add_file, path1)
          file_selector.send(:add_file, path2)
      ...
      it "should add absolute path file" do
        ...
        Dir.chdir(tmpdir) do
          expect { file_selector.send(:add_file, relative_path) }.to output(message).to_stderr
        end
        expect(files).to contain_exactly(absolute_path)
      end
    end
    ...
    describe ".select_files" do
      it "should return empty array" do
        ...
        expect(file_selector_class.select_files).to be_empty
      end
      ...
      it "should allow to input multiple files" do
        ...
        allow(Readline).to receive(:readline) { inputs.next }
        ret = nil
        expect { ret = file_selector_class.select_files }.to output(message).to_stderr
    ...
    describe ".select_file" do
      it "should add a file" do
        ...
        ret = nil
        expect { ret = file_selector_class.select_file }.to output(message).to_stderr
      ...
      it "should reject multiple files in one line" do
        ...
        inputs = [[path1, path2].shelljoin, nil].to_enum
        allow(Readline).to receive(:readline) { inputs.next }
        expect { file_selector_class.select_file }.to output(message).to_stderr \
      ...
      it "should be needed to input a file" do
        inputs = [nil].to_enum
        allow(Readline).to receive(:readline) { inputs.next }
        expect { file_selector_class.select_file }.to raise_error RuntimeError
      end
